...
const API_URL = 'http://localhost:56298/api';
...
export class RestCommunicationManager implements ICommunicationManager {
    ...
    getAlgorithms(): Observable<IAlgorithm[]> {
        ...
        return this.http.get(API_URL + '/Algorithms')
                        .map(response => {
                        const algorithms = response.json();
                        return algorithms.map((algo) => new Algorithm(algo));
                        })
